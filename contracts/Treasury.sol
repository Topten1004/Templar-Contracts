...
contract Treasury is Ownable {
    ...
    address public SWORD;
    uint public SWORDQueue; // Delays change to SWORD address
    ...
    /**
        @notice allow approved address to deposit an asset for TEM
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint TEM needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( TEM ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn TEM for reserves
        @param _amount uint
        @param _token address
     */
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( SWORD ).balanceOf( msg.sender ); // Can only borrow against SWORD held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with TEM
        @param _amount uint
     */
    function repayDebtWithTEM( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ITEMERC20( TEM ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, TEM, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( TEM ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( TEM ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns TEM valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match TEM decimals
            value_ = _amount.mul( 10 ** IERC20( TEM ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SWORD ) { // 9
            SWORDQueue = block.number.add( blocksNeededForQueue );
        } else return false;
